...
class process_scans():
    ...
    def fslsplit(self):
        ...
        # split_vols = glob(os.path.join(self.home_dir,'vol*')) # Why FSL
        # insists on putting this here, I don't know.
    ...
    def find_vols(self, vols=246):
        ...
        if min_extra_vols == 0 or not min_extra_vols:
            ...
            print(
                '{} volumes available.\n{} volumes of bad data in those first files.'.format(
                    len(mdata), min_extra_vols))
            while min_extra_vols > 0 and min_extra_vols <= vols:
                ...
                # update to include the current last volume, so that another
                # iteration can pickup the outlier search where this loop
                # leaves off
                vols = int(end_vol + min_extra_vols)

        print('Checking if the number of volumes ({}) is greater than needed ({})'.format(
            len(self.split_vols), vols))
        if len(self.split_vols) >= vols:
            extras_dir = self.rest_dir + '_extraVols'
            print(
                'Creating extras directory, {}, and moving {} files to it.'.format(
                    extras_dir, len(mdata) - vols))
            ...
            for vol in self.split_vols:
                ...
                if int(
                        v) > vols:  # don't make it = to vols, b/c want to be volume number inclusive
                    try:
                        shutil.move(
                            vol, os.path.join(
                                extras_dir, vol_filename))
