...
class bnv_analysis():
    def __init__(
            ...
            label_file=os.path.join(
                shared.atlas_dir,
                'hcpmmp1_expanded_labels.csv'),
            group=None,
            atlas_label="SuttonLabel",
            subject_list=None,
            prop_thr=0.7,
            prop_thr_pop='sample',
            exclude_negatives=shared.excl_negatives):
        ...
        if prop_thr_pop == 'sample':
            # Can be 'sample' for the whole popultion or anything else to
            # trigger self.group as the input
            self.prop_thr_pop = []
    ...
    def limit_labels(self, network=None):
        ...
        return self.label_df.loc[self.label_df[self.atlas_label].str.lower().isin(
            parcels)]
    ...
    def load_summary_data(self, analyze_group=None, bnv_node=True):
        ...
        if not analyze_group:
            grp_dict = {
                'shared.group1': shared.group1,
                'shared.group2': shared.group2}
        else:
            ...
            grp_dict = {
                k: v for k,
                v in shared.__dict__.items() if str(v) in analyze_group}

        dfs = []
        for k in grp_dict.keys():
            # Flexible solve for group 1 or 2, depending on the group id from
            # __init__
            indices = shared.__dict__[k.split('.')[-1] + '_indices']
            print(indices)
        ...
        if compare == 'yes':
            ...
            df = df.groupby('rois').mean().reset_index().rename(
                columns={'index': 'rois'})
        return df
    ...
    def get_cohort_bnv_data(self, indices=[1], mean=False):
        network_mask = fam.make_proportional_threshold_mask(
            self.network,
            self.prop_thr,
            exclude_negatives=self.exclude_negatives,
            subset=self.prop_thr_pop)
        parcels = get.get_network_parcels(self.network)
        subj_dfs = []
        data = get.get_cohort_network_matrices(
            self.network,
            indices,
            mean=False,
            conn_data=None,
            prop_thr=self.prop_thr,
            subject_level=False,
    ...
    def calc_diff_df(self, orig_df):
        ...
        df1 = orig_df.loc[orig_df['group'] == groups[0], :].drop(
            columns=['group']).groupby('rois').mean()
        df2 = orig_df.loc[orig_df['group'] == groups[1], :].drop(
            columns=['group']).groupby('rois').mean()
        df = df1 - df2
    ...
    def make_node_file(self, msr_of_int='fc', analyze_group=None):
        analyze_group = self.group if analyze_group is None else analyze_group
        fc_df = self.load_summary_data(
            analyze_group=analyze_group, bnv_node=True)
        node_df = fc_df[[msr_of_int] + ['rois']]
        out_df = self.label_df.merge(node_df,
                                     left_on=self.atlas_label,
                                     right_on='rois').drop(columns=([self.atlas_label]))
        out_df['size'] = out_df[msr_of_int]
        out_df = out_df.apply(lambda x: x.str.strip()
                              if x.dtype == "object" else x)
        out_df = out_df[['x', 'y', 'z', msr_of_int, 'size', 'rois']]
        out_df.drop_duplicates(inplace=True)
        out_df = out_df.replace({np.nan: 0})
        out_df.to_csv(os.path.join(shared.conn_dir, str(shared.date) + '_' + self.network + '_' + ''.join(
            self.group) + '_' + str(self.prop_thr).split('.')[-1] + '_bnv.node'), header=False, index=False, sep='\t')
    ...
    def make_edge_file(self, analyze_group=None, mdata=None):
        ...
        indices = list(parcels.values())
        edges = self.load_summary_data(
            analyze_group=analyze_group, bnv_node=False)
        drop_cols = edges.columns.intersection(
            [shared.name_id_col, shared.group_id_col, 'rois', 'subj_num'])
        if len(drop_cols) > 0:
